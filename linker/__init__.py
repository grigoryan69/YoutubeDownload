...
_linker_js_file = pkg.resource_filename('linker', 'linker.js')
_linker_js = open(_linker_js_file, encoding='utf-8').read()
_websockets = []
_call_return_values = {}
...
# Can be overridden through `linker.init` with the kwarg `js_result_timeout` (default: 10000)
...
_start_args = {
    ...
    # Hostname use for Bottle server
    'host':             'localhost',
    # Port used for Bottle server (use 0 for auto)
    'port':             8000,
    # Whether start() blocks calling thread
    'block':            True,
    ...
    'size':             None,
    # (left, top) of main window
    'position':         None,
    # Dictionary of size/position for all windows
    'geometry':         {},
    # Callback for when all windows have closed
    'close_callback':   None,
    'app_mode':  True,                              # (Chrome specific option)
    # Allow bottle server to listen for connections on all interfaces
    'all_interfaces': False,
    # Sets the no-store response header when serving assets
    'disable_cache': True,
...
api_error_message = '''
----------------------------------------------------------------------------------
  'options' argument deprecated in v1.0.0, see https://github.com/ChrisKnott/linker
...
def expose(name_or_function=None):
    # Deal with '@linker.expose()' - treat as '@linker.expose'
    if name_or_function is None:
        return expose

    if type(name_or_function) == str:   # Called as '@linker.expose("my_name")'
...
def start(*start_urls, **kwargs):
    ...
    if _start_args['jinja_templates'] != None:
        ...
        templates_path = os.path.join(
            root_path, _start_args['jinja_templates'])
...
def _linker():
    ...
    page = _linker_js.replace('/** _py_functions **/',
...
BOTTLE_ROUTES = {
    "/linker.js": (_linker, dict()),
    "/<path:path>": (_static, dict()),
    "/linker": (_websocket, dict(apply=[wbs.websocket]))
}
