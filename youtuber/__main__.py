"""
This module implements the core developer interface for youtuber.

The problem domain of the :class:`YouTube <YouTube> class focuses almost
exclusively on the developer interface. youtuber offloads the heavy lifting to
...
import youtuber
import youtuber.exceptions as exceptions
from youtuber import extract, request
from youtuber import Stream, StreamQuery
from youtuber.helpers import install_proxy
from youtuber.innertube import InnerTube
from youtuber.metadata import YouTubeMetadata
from youtuber.monostate import Monostate

logger = logging.getLogger(__name__)


class YouTube:
    """Core developer interface for youtuber."""
    ...
    def __init__(
        ...
        :param func on_complete_callback:
            ...
            complete events.
        :param dict proxies:
            (Optional) A dict mapping protocol to proxy address which will be used by youtuber.
    ...
    def __repr__(self):
        return f'<youtuber.__main__.YouTube object: videoId={self.video_id}>'
    ...
    def js(self):
        ...
        if youtuber.__js_url__ != self.js_url:
            self._js = request.get(self.js_url)
            youtuber.__js__ = self._js
            youtuber.__js_url__ = self.js_url
        else:
            self._js = youtuber.__js__
    ...
    def fmt_streams(self):
        ...
        # https://github.com/youtuber/youtuber/issues/1054
        try:
            ...
            self._js = None
            self._js_url = None
            youtuber.__js__ = None
            youtuber.__js_url__ = None
            extract.apply_signature(stream_manifest, self.vid_info, self.js)
    ...
    @property
    def caption_tracks(self) -> List[youtuber.Caption]:
        ...
        return [youtuber.Caption(track) for track in raw_tracks]

    @property
    def captions(self) -> youtuber.CaptionQuery:
        """Interface to query caption tracks.

        :rtype: :class:`CaptionQuery <CaptionQuery>`.
        """
        return youtuber.CaptionQuery(self.caption_tracks)
    ...
    def title(self) -> str:
        ...
        try:
            ...
            self.check_availability()
            raise exceptions.youtuberError(
                (
                    f'Exception while accessing title of {self.watch_url}. '
                    'Please file a bug report at https://github.com/youtuber/youtuber'
                )
            )
