"""Various helper functions implemented by youtuber."""
import functools
import gzip
import json
import logging
...
from youtuber.exceptions import RegexMatchError
...
class DeferredGeneratorList:
    ...
    youtuber has some continuation generators that create web calls, which means
...
def setup_logger(level: int = logging.ERROR, log_filename: Optional[str] = None) -> None:
    ...
    # https://github.com/youtuber/youtuber/issues/163
...
def create_mock_html_json(vid_id) -> Dict[str, Any]:
    ...
    # Get the youtuber directory in order to navigate to /tests/mocks
    youtuber_dir_path = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__),
            os.path.pardir
        )
    )
    youtuber_mocks_path = os.path.join(youtuber_dir_path, 'tests', 'mocks')
    gzip_filepath = os.path.join(youtuber_mocks_path, gzip_filename)
